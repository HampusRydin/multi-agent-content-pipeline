...
Write the full blog post now - do not summarize or abbreviate."""
    ...
    def run(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self._log_to_supabase(prd, draft_content, metrics, post_id)
    ...
    async def run_async(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        post_id = state.get("post_id")
        if post_id is None:
            print(f"\033[93m[WARNING]\033[0m Writer (async): post_id is None in state! Logs will not be linked.")
        else:
            print(f"\033[94m[DEBUG]\033[0m Writer (async): Logging with post_id={post_id}")
        if self.supabase:
